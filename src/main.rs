...
fn rand_t(rng: &mut RuntimeRng, shape: &[usize], device: &Device) -> CResult<Tensor> {
    let n: usize = shape.iter().product();
...
impl CriticalityEstimator {
    fn update(&mut self, movement: f32) -> f32 {
        ...
        let slope = |lag: usize| -> Option<f32> {
            let pairs = x.len() - lag;
            let mean_prev = x[..pairs].iter().sum::<f32>() / pairs as f32;
            let mean_next = x[lag..].iter().sum::<f32>() / pairs as f32;
            let mut cov = 0.0;
            let mut var_prev = 0.0;
            for i in 0..pairs {
                let a = x[i] - mean_prev;
                let b = x[i + lag] - mean_next;
                cov += a * b;
                var_prev += a * a;
            }
            (var_prev > 1e-9).then_some(cov / var_prev)
        };
        let per_step = |lag: usize| -> Option<f32> {
            let value = slope(lag)?;
            (value > 0.0 && value.is_finite()).then_some(value.powf(1.0 / lag as f32))
        };
        let estimates = [per_step(1), per_step(2), per_step(4)];
        let valid: Vec<f32> = estimates.into_iter().flatten().collect();
        if valid.len() < 2 {
            self.confidence *= 0.92;
            return self.sigma;
        }
        let estimate = (valid.iter().sum::<f32>() / valid.len() as f32).clamp(0.0, 1.5);
        let spread = valid.iter().map(|v| (v - estimate).abs()).sum::<f32>() / valid.len() as f32;

        // A branching estimate is meaningful only while the measurement process is
        // approximately stationary and has enough dynamic range. Compare adjacent
        // halves rather than trusting a high autocorrelation caused by slow drift.
        let half = x.len() / 2;
        let half_mean = |slice: &[f32]| slice.iter().sum::<f32>() / slice.len() as f32;
        let m0 = half_mean(&x[..half]);
        let m1 = half_mean(&x[half..]);
        let mean = half_mean(&x);
        let variance = x.iter().map(|v| (v - mean).powi(2)).sum::<f32>() / x.len() as f32;
        let dynamic_range = (variance.sqrt() / (mean.abs() + 1e-4) / 0.15).clamp(0.0, 1.0);
        let stationarity = (1.0 - (m1 - m0).abs() / (variance.sqrt() + 1e-4)).clamp(0.0, 1.0);
        let agreement = (1.0 - spread / 0.25).clamp(0.0, 1.0);
        let sample_support = ((x.len() - 16) as f32 / 64.0).clamp(0.0, 1.0);
        let confidence = dynamic_range * stationarity * agreement * sample_support;

        let update_rate = 0.04 + 0.12 * confidence;
        self.sigma += update_rate * (estimate - self.sigma);
        self.correlation += 0.10 * (valid[0].clamp(-1.0, 1.0) - self.correlation);
        self.confidence += 0.10 * (confidence - self.confidence);
        self.sigma
    }
}
...
// Remove overall log-energy and normalize spectral contrast. Novelty should
// reward a changed spectral shape, not a gain change or mastering drift.
fn normalize_spectral_shape(spectrum: &Tensor) -> CResult<Tensor> {
    let centered = spectrum.broadcast_sub(&spectrum.mean_keepdim(D::Minus1)?)?;
    let scale = centered
        .sqr()?
...
impl PotentialController {
    ...
    fn update(&mut self, state: PotentialState) -> PotOut {
        let PotentialState {
            micro_amp: a_mic,
            macro_amp: a_mac,
            coupling,
...
impl ShearField2D {
    fn new(channels: usize, h: usize, w: usize) -> Self {
        let cl = channels * h * w;
...
fn levy_radiate(tape: &Tensor, amp: f32, rng: &mut RuntimeRng) -> CResult<Tensor> {
    let dev = tape.device();
    let shape: Vec<usize> = tape.dims().to_vec();
    let n1 = randn_t(rng, &shape, 1.0, dev)?;
    let n2 = randn_t(rng, &shape, 1.0, dev)?.abs()?.affine(1.0, 1e-3)?;
    let cauchy = n1.broadcast_div(&n2)?.clamp(-CAUCHY_CLAMP, CAUCHY_CLAMP)?;
    let mask = rand_t(rng, &shape, dev)?;
    let mask_t = mask
...
fn restore_novelty(values: &[Vec<f32>], device: &Device) -> Result<VecDeque<Tensor>> {
    ...
    {
        if v.len() == SPEC_BINS {
            let spectrum = Tensor::from_vec(v.clone(), (1, SPEC_BINS), device)?;
            out.push_back(normalize_spectral_shape(&spectrum)?.detach());
        }
    }
    Ok(out)
}
...
\nCtrl-C finishes the active chunk, finalizes audio, and saves the organism.\n"
    ...
    let available_threads = std::thread::available_parallelism()
        .map(|n| n.get())
        .unwrap_or(8);
    if !target_lr.is_finite() || target_lr <= 0.0 || target_lr > 0.1 {
        anyhow::bail!("learning rate must be finite and in (0, 0.1]");
    }
    if !sim_duration.is_finite() || sim_duration <= 0.0 || sim_duration > 3600.0 {
        anyhow::bail!("duration must be finite and in (0, 3600] seconds");
    }
    ...
    if !fresh_world && std::path::Path::new(&load_world_path).exists() {
        match load_world(&load_world_path) {
            Ok(world) => {
                ...
                loaded_world = true;
                println!("--> Resumed world {} at global step {} (L{:02}, motifs {}, raw/effective model confidence {:.2}/{:.2}).",
                    load_world_path, global_step, model.depth(), motifs.entries.len(),
    ...
    for step in 0..total_chunks {
        ...
        // Compact model-based planning is amortized: one batched prediction
        // for all ten actions every PLAN_EVERY chunks. Between plans, the
        // model-free values continue adapting each chunk.
        if absolute_step.is_multiple_of(PLAN_EVERY as u64) {
            match plan_action_scores(
                &monitor_head,
                &hidden_mem.detach(),
                &device,
                &adaptive_dynamics,
        ...
        let motif_available = motifs.has_recallable(absolute_step);
        let current_action =
        ...
        // This is a residual intervention, not a hard world reset.
        ...
        let mono_spec = normalize_spectral_shape(&out_spec_l.add(&out_spec_r)?.affine(0.5, 0.0)?)?; // (1, bins), level-invariant spectral shape
        ...
        let pot = potential.update(PotentialState {
            micro_amp: micro_abs,
            macro_amp: macro_abs,
            coupling: synergy_val,
            movement,
            energy: energy_state,
            sigma,
            curiosity: recursive_curiosity,
        ...
        if absolute_step.is_multiple_of(EPI_SNAP_EVERY as u64) && absolute_step > 0 {
            episodic.snapshot(&refined_hidden);
        }
        // Novelty buffer cadence.
        if absolute_step.is_multiple_of(NOVELTY_EVERY as u64) {
        ...
        for i in 0..CHUNK_SIZE {
            ...
            // Stateful DC blocker is part of the observed signal path, so the
            // recursive controller hears the same waveform later mastered.
            let xl = audio_l[i];
            let yl = xl - dc_x1_l + dc_pole * dc_y1_l;
            dc_x1_l = xl;
            dc_y1_l = yl;
            audio_l[i] = yl;
            let xr = audio_r[i];
            let yr = xr - dc_x1_r + dc_pole * dc_y1_r;
            dc_x1_r = xr;
            dc_y1_r = yr;
            audio_r[i] = yr;
            raw_peak = raw_peak.max(yl.abs()).max(yr.abs());
        }
        ...
        if step % 10 == 0 {
            ...
            uncertainty_trace.push(serde_json::json!({
                ...
                "pe16": s_sig["pe16"].as_f64().unwrap_or(0.0), "pi_proxy": s_sig["pi_proxy"].as_f64().unwrap_or(0.0),
                ...
                "reward": reward, "recurrence": recurrence,
                "model_confidence": controller.meta.confidence,
                "effective_model_weight": adaptive_dynamics.effective_model_weight,
                "prediction_error": controller.meta.error_ema,
                "calibration_error": controller.meta.calibration_error,
                "region_change": region_change_mean,
                "observation_delta": observation_delta,
                "activity_health": adaptive_dynamics.activity_health,
                "stagnation": adaptive_dynamics.stagnation,
                "low_motion_run": adaptive_dynamics.low_motion_run,
                ...
                "motif_rejected_quality": motif_diagnostics.rejected_quality,
        ...
        if step % 50 == 0 {
            ...
            println!("Chunk {}/{} (global {}) [SPS: {:.2}] | Move: {:.3} | Mimic: {:.3} {} | {} | L{:02} rad:{:.2} | σ:{:.3}/{:.2} | V:{:.3} T:{:.2} | LRx:{:.2}",
                step, total_chunks, absolute_step, rolling_sps, movement, mimic_drift_n, trend,
                current_action.label(), model.depth(), rad_amp, sigma, criticality.confidence,
                pot.v, pot.temp, latest_lr_gain);
            println!("  field H:{:.2}b · {} · synergy:{:.2} empower:{:.2} | energy:{:.2} | φ:{:.2} PI:{:.2} | model raw/eff:{:.2}/{:.2} err:{:.3}",
            ...
            println!(
                ...
                controller.action_age,
                adaptive_dynamics.action_use_ema[current_action.index()]
            );
    ...
    for w in 0..32usize {
        let start = (w * n_frames.saturating_sub(CHUNK_SIZE)) / 31usize;
        ...
        for (k, bin) in buf.iter().enumerate().take(CHUNK_SIZE / 2).skip(1) {
            let m = bin.norm();
    ...
    let prompt = format!(
        ...
        if avg_sigma > 0.97 && avg_sigma < 1.03 { "Critical-Edge Emergence" } else { "Information-Theoretic Glitch" },
...
mod tests {
    ...
    fn criticality_estimator_rejects_flat_activity() {
        ...
        assert!(estimator.confidence < 0.05);
        assert!(estimator.sigma.is_finite());
    }

    #[test]
    fn criticality_estimator_tracks_stationary_propagation() {
        let mut estimator = CriticalityEstimator::default();
        let mut value = 0.02f32;
        for i in 0..256 {
            let forcing = ((i as f32 * 1.731).sin() + (i as f32 * 0.417).cos()) * 0.0015;
            value = 0.82 * value + 0.0036 + forcing;
            estimator.update(value.max(0.0));
        }
        assert!(estimator.confidence > 0.05);
        assert!((0.55..1.05).contains(&estimator.sigma));
    }

    #[test]
    fn spectral_shape_normalization_is_gain_invariant() -> Result<()> {
        let device = Device::Cpu;
        let a = Tensor::new(&[[1.0f32, 2.0, 4.0, 8.0]], &device)?.log()?;
        let b = Tensor::new(&[[4.0f32, 8.0, 16.0, 32.0]], &device)?.log()?;
        let distance = normalize_spectral_shape(&a)?
            .sub(&normalize_spectral_shape(&b)?)?
            .abs()?
            .max_all()?
            .to_scalar::<f32>()?;
